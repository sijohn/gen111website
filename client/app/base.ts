...
abstract class BaseCtrl {
  ...
  clone(item: any) {
    if (!item) { return; }
    this.modal.confirm('Copy', 'Would you like to copy the ' + this.api + '?').subscribe(result => {
      if (result) {
        let clone = _.cloneDeep(item);
        delete clone._id;
        this.crud.add(this.api, clone).subscribe(data => { this.flush(); this.filterText = ''; this.getData() }, err => this.snack.open(err, 'OK', { duration: 2000 }));
      }
    })
  }
  delete(item: any) {
    if (!item) { return; }
    this.modal.confirm('Delete?', 'Would you like to delete the ' + this.api + '?').subscribe(result => {
      if (result) {
        this.crud.delete(this.api, item._id).subscribe(data => { this.flush(); this.filterText = ''; this.getData() }, err => this.snack.open(err, 'OK', { duration: 2000 }));
      }
    })
  }
  getData(q?: any, scrolled?: boolean) {
    ...
    if (q.search || this.meta.search) {
      this.meta.search = q.search; params.set('search', JSON.stringify({ name: q.search }))
    }
